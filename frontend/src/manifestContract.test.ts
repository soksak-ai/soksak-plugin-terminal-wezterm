...
describe("terminal plugin manifest contract", () => {
  it("declares every common terminal command", () => {
    ...
    expect(manifest.runtimeDependencies.sidecars.map((sidecar: { id: string }) => sidecar.id)).toEqual(["soksak-sidecar-pty", "soksak-sidecar-terminal-wezterm"]);
